...
`
  ...
  describe('global lock dangling cleanup', () => {
    ...
    beforeEach(() => {
      ...
      mkdirSync(join(tempHome, '.local', 'state', 'skills'), { recursive: true });
    });
    ...
    it('purges a dangling global lock entry whose files are not on disk', () => {
      ...
      // runCli also points XDG_STATE_HOME at <HOME>/.local/state, and
      // getSkillLockPath() prefers that over ~/.agents, so write the lock there.
      const lockPath = join(tempHome, '.local', 'state', 'skills', '.skill-lock.json');
      writeFileSync(
        lockPath,
        JSON.stringify(
          {
